...
\Author{\hl{Polina Lemenkova} \orcidA{}}
...
However, recent monitoring has indicated their decline, habitat loss, and environmental vulnerability~\cite{BASTOS2023102144,SUWANDHAHANNADI2024107069,MOHAPATRA2023163109}. These declines have been attributed to a variety of indirect and direct causes, including climate change~\cite{SIMANTIRIS2023108231} and anthropogenic activities with their associated pollution~\cite{GHANDOURAH2023102982}. High levels of stress result  in   environmental threats to these treasured ecosystems~\cite{Davis}. Among recent environmental problems of coastal lagoons are affected biodiversity patterns and loss of habitats and rare species~\cite{Kumari2022}, as well as disrupted land cover patterns due to climate effects. Examples of pollution include organic, chemical, and biological types such as microplastic~\cite{GARCESORDONEZ2022120366,BRUSCHI2023110596} or heavy metals~\cite{Tripathi2022}. 
...
%    \setlength{\unitlength}{0.5cm}
   ...
   % \end{center}
    \caption{\hl{Topographic}  map of India with indicated study area showing the location of the Landsat data over the coastal lagoon of Chilika Lake. Software: GMT. Map source: author.}
%MDPI: Figures 1 and 2 have same captions. Please check and revise if necessary.
...
%    \setlength{\unitlength}{0.5cm}
  ...
  %  \end{center}
    \caption{Flowchart showing the methodological scheme used in this research. Software: \hl{RStudio.} Diagram source: author.}
...
\begin{lstlisting}[captionpos=t,language=bash,caption=\hl{GRASS} %MDPI: We formatted the all listings to  standard format. Please confirm. 
...
\begin{lstlisting}[captionpos=t,language=bash,caption=\hl{GRASS}  
...
%    \setlength{\unitlength}{0.5cm}
   ...
   % \end{center}
    \caption{General methodological scheme for the Artificial Neural Network (ANN) used for image classification. Software:\hl{ R, library ``grViz''.} Diagram source: author.}
    %MDPI: Please state the version number of the software. If it is not applicable, the related website or proof should be provided.
    \label{fig06}
\end{figure}

Using ML modules of GRASS GIS, the image classification using the MLPC algorithm was implemented using the combination of the modules ``r.learn.train'' used for extracting training data, supervised machine learning, and cross-validation using the Python package Scikit-Learn, and the module ``r.learn.predict'' for estimating prediction of pixels' classification. The technical implementation was performed using    the following code: 

\begin{lstlisting}[captionpos=t,language=bash,caption=\hl{GRASS} %MDPI: Listing 4 and 5 have same labels {lst04}. Please check and revise. Please mention Listing 4 before and close to where it is first presented in the text, and make sure all lstlistings are mentioned in numerical order.
...
\begin{table}[H] 
\caption{\hl{Estimated} %MDPI: Table 1 and Table A2 have same labels {tab01}. Please check and revise. Please mention Table 1 before and close to where it is first presented in the text, and make sure all tables are mentioned in numerical order.
...
\toprule
 \multirow{2}{*}{\textbf{Year}} & \multicolumn{10}{c}{\textbf{Classes of Land Cover Types}} \\
    ...
    \midrule
    \hl{\textbf{2019}} %MDPI: Please confirm if the bold is unnecessary and can be removed. The following highlights are the same.
...
\begin{figure}[H]
  ...
  \begin{tabular}[b]{c}
    ...
    \small (\textbf{f}) 2024
  \end{tabular}
  \caption{\hl{Image processing} 
  %MDPI: Please change the hyphen (-) into a minus sign ($-$, "U+2212") in image if possible. e.g., "-1" should be "$-$1". The following highlights are the same.
...
\begin{figure}[H]
 % \centering
  ...
  \begin{tabular}[b]{c}
    ...
    \small (\textbf{f}) 2024
  \end{tabular}
  \caption{\hl{Random} Forest ML-based classification of the Landsat 8 OLI/TIRS scenes from 2019 to 2024. Background shaded relief: GEBCO grid. Software: GRASS GIS. Mapping source: author.}
  \label{fig08}
\end{figure}
...
%  \centering
  ...
  \begin{tabular}[b]{c}
    ...
    \small (\textbf{f}) 2024
  \end{tabular}
  \caption{\hl{Support} Vector Machine (SVM) ML-based classification of the Landsat 8 OLI/TIRS scenes from 2019 to 2024. Background shaded relief: GEBCO. Software: GRASS GIS. Maps source: author.}
  \label{fig09}
\end{figure}
...
\begin{figure}[H]
 % \centering
  ...
  \begin{tabular}[b]{c}
    ...
    \small (\textbf{f}) 2024
  \end{tabular}
  \caption{\hl{MLPClassifier} ANN-based classification of the Landsat 8 OLI/TIRS scenes from 2019 to 2024. Background shaded relief: GEBCO grid. Software: GRASS GIS. Mapping source: author.}
  \label{fig10}
\end{figure}
...
\begin{figure}[H]
 % \centering
  ...
  \begin{tabular}[b]{c}
    ...
    \small (\textbf{f}) 2024
  \end{tabular}
  \caption{\hl{Accuracy} analysis of image classification using algorithm of reject threshold probability by chi square test. Background relief: GEBCO. Software: GRASS GIS. Mapping source: author.}
  \label{fig11}
\end{figure}

Accordingly, the stability of points assigned to land cover classes were evaluated and reported as follows for each year: for 2019, 98.08\% points stable; for 2020, 98.23\%; for 2021, 98.19\%; for 2022, 98.25\%; for 2023, 98.37\%; and for 2024, 98.08\% points stable, respectively. Finally, class means and standard deviations computed for each band and each year, respectively, are presented in the Appendix~\ref{AppB} for the years 2019 to 2024. The maps of accuracy analysis assessment  of image classification has been performed using rejection-threshold probability techniques using chi square test, the results of  which are  presented in Figure~\ref{fig11}. 

The final results of the computations included the convergence for iterations of pixels assigned to the land cover classes. The convergence was computed for each image and demonstrated the following results: for the year 2019, 98.1\%; for the year 2020, 98.2\%; for the year 2021, 98.2\%; \hl{for the year 2022, \%; } %MDPI: Please check if any date is missing.
...
Change-detection analysis in the land cover types from 2013 to 2023 demonstrated shifts in land cover types within the basin of the Chilika Lake coastal lagoon. The computed areas occupied by various land cover classes for each of the category are summarized in Table~\ref{tab01}. Such variability of landscape patches around the Chilika Lake proved the existence of  fluctuations in the processes of siltation and eutrophication in the coastal lagoon over the studied period (years 2019--2024). Major causes of such environmental changes are related to   sedimentation processes such as low river flow speed, which cause stagnation of water and eutrophication. Second, the increased sediment budgetary balance and flushing of sediments into the Bay of Bengal also contribute to the changes in the lake level. Since water fluctuations in various parts of the Chilika Lake are subject to   monsoon effects in dry and wet seasons, wetland types can be discriminated on the satellite images along the margin areas of the lagoon. These variations are also caused by the different hydrological effects (e.g., currents and water level) and geomorphic settings of the surrounding relief. 
 ...
 Table~\ref{tab02} summarizes and compares such parameters as accuracy, F1 score, Cohen's kappa coefficient, and related parameters. Based on the computed values of Cohen's Kappa coefficient and the F1 method for the evaluation of classified data, the results suggest that the 0.785 Cohen's Kappa coefficient and F1 score of 0.89 are rated as the good strength of agreement. Hence, the ANN technique can be interpreted as a reliable approach for satellite image classification in terms of accuracy, followed by the SVM and RF. The interpretation of the  F score is as follows. The highest possible value of 1.0  indicates the perfect precision and recall of variables, while the lowest theoretically possible value of 0 indicates the lowest precision or recall which are zero in this case. Hence,  higher values generally mean better results of satellite image classification. Likewise, as other correlation statistics, the kappa can range from {$-$}1 to +1. The interpretation of values is as follows. Values of the levels of 0.60 to 0.79 have  a moderate level of agreement, those in the interval of 0.8--0.9 (that is, over 80\% below 90\%) are acceptable as having strong level of agreement, and those above 0.9 are almost perfect~\highlighting{\cite{McHugh}.}%MDPI: Refs. [101] and [102] are not cited in main text. Please check and revise.
...
\begin{adjustwidth}{-\extralength}{0cm}
    ...
    \begin{tabularx}{\fulllength}{Lm{4cm}<{\raggedright}m{4cm}<{\raggedright}m{4cm}<{\raggedright}}
    \toprule
        	\textbf{Data Set Attribute} & \textbf{Attribute Value} & \textbf{Attribute Value} & \textbf{Attribute Value} \\ \hline
        Landsat Scene Identifier %MDPI: Please confirm if the bold is unnecessary and can be removed. The following highlights are the same. %Author: yes, bold can be removed (I removed it).
& LC81400462019044LGN00 & LC81400462020063LGN00 & LC81400462021033LGN00 \\ \hline
        ...
        Scene Center Lat DMS & "20°13'48.04""N" & "20°13'47.89""N" & "20°13'47.39""N" \\ \hline
...
\begin{table}[H]
\caption{\hl{Class means} %MDPI: Table 1 and Table A2 have same labels {tab01}. Please check and revise.
 of Digital Numbers (DNs) of pixels computed for pixels assigned to 10 land cover classes in the landscapes around Chilika Lake within each Landsat 8-9 OLI/TIRS band (1 to 7) for the years 2019 to 2024.\label{tab01}}
	\begin{adjustwidth}{-\extralength}{0cm}
		\newcolumntype{C}{>{\centering\arraybackslash}X}
		\newcolumntype{L}{>{\raggedright\arraybackslash}X}
...
\newblock Chapter 15---Coastal Lagoons: Environmental Variability, Ecosystem
  ...
  Wolanski, E., Day, J.W., Elliott, M., Ramachandran, R., Eds.; Elsevier:  \hl{Amsterdam, The Netherlands,} 2019; pp. 253--276.
  %MDPI: Newly added and/or altered information. Please confirm. The following highlights are the same.
...
\newblock Comparative Oceanography of Coastal Lagoons. In {\em Estuarine
  Variability}; Wolfe, D.A., Ed.; Academic Press:  \hl{Cambridge, MA, USA,} 
1986; pp. 63--81.
\newblock
...
Davis, R.A., Jr.; FitzGerald, D. Coastal Lagoons. In {\em Beaches and Coasts}; John Wiley \& Sons, Ltd.: \hl{Hoboken, NJ, USA,} %
...
\newblock In {\em Phycotoxins}; John Wiley \& Sons, Ltd.:  \hl{Hoboken, NJ, USA,} %
...
\newblock Chapter 13 Fisheries In Coastal Lagoons. In {\em Coastal Lagoon
  ...
  Oceanography Series; Elsevier:  \hl{Amsterdam, The Netherlands,} %
...
\newblock {\em {Geographic Resources Analysis Support System (GRASS GIS)
  Software}}, Version 8.2; Open Source Geospatial Foundation: \hl{Beaverton, OR, USA} 
,  2022.
...
\newblock In Proceedings of the IGARSS 2022---2022 IEEE International
  Geoscience and Remote Sensing Symposium, \hl{Kuala Lumpur, Malaysia, 17--22 July}  
 2022; pp. 2876--2879.
...
\newblock In Proceedings of the 2014 The Third International Conference on
  Agro-Geoinformatics, \hl{Beijing, China, 11--14 August}  
 2014; pp. 1--5.
...
\newblock In Proceedings of the 2021 IEEE International Geoscience and Remote
  Sensing Symposium IGARSS, \hl{Brussels, Belgium, 11--16 July}  
 2021; pp. 8550--8553.
...
\newblock In Proceedings of the 2023 International Conference on Earth
  Observation and Geo-Spatial Information (ICEOGI), \hl{Algiers, Algeria, 22--24 May} 
 2023; pp. 1--4.
...
\newblock In Proceedings of the 2011 IEEE International Geoscience and Remote
  Sensing Symposium, \hl{Vancouver, BC, Canada, 24--29 July} 
 2011; pp. 3464--3467.
...
\newblock In Proceedings of the 2020 4th International Conference on
  Electronics, Communication and Aerospace Technology (ICECA), \hl{Coimbatore, India, 5--7 November } 2020; pp.~946--953.
...
\newblock {\em Remote Sens.} {\bf 2024}, {\em 16}, \hl{127.}  
...
\newblock {\em Remote Sens.} {\bf 2023}, {\em 15}, \hl{1148}. 
...
\newblock {Chapter 7 - Google Earth Engine and machine learning classifiers for
  ...
  Kalogeropoulos, K., Eds.; Earth Observation, Elsevier:  \hl{Amsterdam, The Netherlands,} %
 2023; pp. 131--148.
...
\newblock In Proceedings of the 3rd International Conference on
  Document Analysis and Recognition, \hl{Montreal, QC, Canada, 14--16 August} 1995; Volume~1, pp. 278--282.
...
\newblock In \emph{Proceedings of the Seventeenth International Florida Artificial Intelligence Research Society Conference (FLAIRS 2004)};  \hl{AAAI Press: Menlo Park, CA, USA}, 2004; pp.~1--6.
...
\newblock In {\em Wetlands Conservation}; John Wiley \& Sons, Ltd.:  \hl{Hoboken, NJ, USA,} 
...
\newblock 19 - The imprints of Holocene climate and environmental changes in
  ...
  Eds.; Elsevier:  \hl{Amsterdam, The Netherlands,} 
  2022; pp. 457–483
...
\newblock Chapter 9---Integrated Approach for Modeling Coastal Lagoons: A Case
  ...
  Part A}; Handbook of Statistics; {Srinivasa Rao}, A.S., Rao, C., Eds.; Elsevier:  \hl{Amsterdam, The Netherlands,} %
...
\newblock Chapter 32---Maintaining ecological resilience on a regional scale:
  ...
  Marine Protected Areas}; Humphreys, J., Clark, R.W., Eds.; Elsevier:  \hl{Amsterdam, The Netherlands,} 
  2020; pp. 631--647.
